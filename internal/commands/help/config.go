...
func ConfigPage() *gohelp.Page {
	return gohelp.NewPage("config", "agent.toml reference").
		Text("The agent.toml file controls your agent's behavior, model selection, interface configuration, and memory limits. Edit this file to switch providers or models, enable specific tools, configure interfaces, or adjust conversation history retention. Changes take effect on the next message (hot-reload enabled).").
