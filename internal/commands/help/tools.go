...
func ToolsPage() *gohelp.Page {
	return gohelp.NewPage("tools", "creating tool definitions").
		...
		Text("If set, 'report' renders after the tool runs and is delivered to the interface the request came from, so tool activity is visible without reading host logs. Delivery passes the family and message as separate values, not a pre-formatted string - each interface renders them its own way (e.g. Telegram: \"[family] message\" in a code block; CLI: \"family:message\"). The family is auto-derived from the tool's folder (tools/memory/read.toml -> \"memory\"); tools directly under tools/ fall back to the tool's own name (tools/weather.toml -> \"weather\"). Report resolution is non-fatal - a broken template logs a warning and is skipped.")
}
