...
- `report`: opt-in per-tool line delivered to the originating interface (Telegram/etc.) when the tool runs, so tool usage is visible without reading host logs. `agent.ToolReport{Family, Message}` carries family and message as separate values (never a pre-formatted string) all the way to the gateway's `Sender.SendToolReport(platformID, family, message)`, so each interface renders them in its own style (Telegram: `[family] message` inside a code block; CLI: `family:message`). Family is auto-derived from the tool's folder (`tools/memory/read.toml` -> `memory`); tools directly in `tools/` fall back to the tool's own name (`tools/weather.toml` -> `weather`). Resolves `{{name}}` (resolved param value, honours return-overrides), `{{$command}}` (fully-resolved shell command), `{{$result}}` (truncated stdout), plus standard directives/system vars. `{{$command}}`/`{{$result}}` are local tokens substituted after `resolution.Process` runs — never part of the global sysvar namespace. Non-fatal: a broken template logs a warning and is skipped.
